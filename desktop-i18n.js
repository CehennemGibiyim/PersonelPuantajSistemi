...
function platformHasUsableTranslations() {
  try {
    const value = window.miniappI18n?.t?.('app.title');
...
export function initDesktopI18n() {
  ...
  catalogPromise = (async () => {
    ...
    // The platform service normally owns static translations. In preview or
    // in an offline desktop copy it can exist only as a blocked/empty stub;
    // use the bundled catalog instead of painting raw keys such as
    // "app.title" or "title" into the page.
    if (!platformHasUsableTranslations()) replaceStaticText();
  })();
