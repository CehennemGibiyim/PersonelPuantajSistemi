...
function buildReport(monthlyNote = '') {
  ...
  for (let day = 1; day <= getDaysInMonth(); day += 1) {
    ...
    html += `<tr class="${classes}"><td>${date}</td><td>${esc(getFullDayName(day))}</td>${columns.map(column => `<td>${cellContent(dayRecords, column)}</td>`).join('')}</tr>`;
  }
  html += `</tbody></table></div>`;
  if (String(monthlyNote).trim()) html += `<section class="duty-print-note"><strong>${esc(t('printOptions.noteLabel'))}:</strong><p>${esc(monthlyNote).replace(/\n/g, '<br>')}</p></section>`;
  html += `<section class="duty-print-approval"><h2>${esc(t('dutySystem.printApproval'))}</h2><div class="duty-print-signatures"><div>${esc(t('dutySystem.approvalSupervisor'))}<br><strong>${esc(admins.headNurse)}</strong></div><div>${esc(t('dutySystem.approvalManager'))}<br><strong>${esc(admins.manager)}</strong></div><div>${esc(t('dutySystem.approvalChief'))}<br><strong>${esc(admins.chiefDoctor)}</strong></div></div></section></div></main>`;
  return html;
}

export async function doDutyPrint() {
  const monthlyNote = await requestMonthlyNote();
  if (monthlyNote === null) return;
