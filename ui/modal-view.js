...
function getUnitSelectorHTML() {
  const units = getUnits();
  const currentId = getCurrentUnitId();
  if (!units.length) return '';
  return `
    <div class="personnel-unit-assignment">
      <div class="modal-label">${t('modal.assignmentUnits')}</div>
      <p class="personnel-assignment-hint">${t('modal.assignmentHint')}</p>
      <div class="personnel-unit-list">
        ${units.map(unit => `<label class="personnel-unit-option${unit.id === currentId ? ' current' : ''}"><input type="checkbox" name="personnelUnits" value="${esc(unit.id)}"${unit.id === currentId ? ' checked' : ''}><span>${esc(unit.name)}</span>${unit.id === currentId ? `<small>${esc(t('modal.currentUnit'))}</small>` : ''}</label>`).join('')}
      </div>
    </div>
  `;
}
...
function getSelectedUnitIds() {
  return [...overlay.querySelectorAll('input[name="personnelUnits"]:checked')].map(input => input.value);
}

// ─── ADD MODE ───

function showAdd(callback = null) {
  currentMode = 'add';
  afterAdd = callback;
  createOverlay();
  buildForm();
...
async function handleConfirm(mode) {
  ...
  if (mode === 'add') {
    ...
    if (!unitIds.length) { showError(t('modal.assignmentRequired')); return; }
...
export { showAdd, showEdit, showDelete };
