...
export function showPersonnelDetail(name, afterSave = null) {
  ...
  overlay.innerHTML = `
    <div class="modal glass" style="max-width:460px">
      ...
      <p style="font-size:12px;color:rgba(255,255,255,0.5);margin-bottom:16px">${esc(typeLabel)}</p>
      ...
      <div style="display:flex;flex-direction:column;gap:14px">
        <div>
          ...
          <input type="text" id="detailEmergency" class="modal-input" style="margin-top:10px" placeholder="${esc(t('detail.emergency'))}" value="${esc(contact.emergency)}" ${!editable ? 'disabled' : ''}>
          <input type="text" id="detailAddress" class="modal-input" style="margin-top:10px" placeholder="${esc(t('detail.address'))}" value="${esc(contact.address)}" ${!editable ? 'disabled' : ''}>
        </div>
        ...
        <div>
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
            <div>
              ...
              <input type="number" id="detailAnnual" class="modal-input" value="${esc(leave.annual)}" min="0" ${!editable ? 'disabled' : ''}>
            </div>
            <div>
              <label class="modal-label" for="detailSick">${t('detail.sick')}</label>
              <input type="number" id="detailSick" class="modal-input" value="${esc(leave.sick)}" min="0" ${!editable ? 'disabled' : ''}>
            </div>
            <div>
              <label class="modal-label" for="detailUnpaid">${t('detail.unpaid')}</label>
              <input type="number" id="detailUnpaid" class="modal-input" value="${esc(leave.unpaid)}" min="0" ${!editable ? 'disabled' : ''}>
            </div>
          </div>
        </div>
      </div>
