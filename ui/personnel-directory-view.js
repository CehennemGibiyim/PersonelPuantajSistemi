...
import { showPersonnelDepartments } from './personnel-departments-view.js';
import { showPersonnelAttendance } from './personnel-attendance-view.js';
...
function photoValue(name) {
  const photo = getPersonnelPhoto(name);
  return photo?.url
    ? `<img class="directory-photo" src="${esc(photo.url)}" alt="${esc(name)}">`
    : `<span class="directory-photo directory-photo-empty" aria-hidden="true">${esc(String(name || '?').slice(0, 1))}</span>`;
}
...
function renderRows(filter = '') {
  ...
  if (!filtered.length) {
    body.innerHTML = `<tr><td colspan="7" class="directory-no-results">${esc(t('directory.noResults'))}</td></tr>`;
    return filtered;
  }
  body.innerHTML = filtered.map(name => {
    const info = getContactInfo(name) || {};
    const type = getPersonnelType(name) === 'civil' ? t('modal.typeCivil') : t('modal.typeWorker');
    return `<tr><td class="directory-photo-cell">${photoValue(name)}</td><td class="directory-name">${esc(name)}<small>${esc(type)}</small></td><td>${contactValue(info.phone)}</td><td>${contactValue(info.email)}</td><td>${contactValue(info.emergency)}</td><td>${contactValue(info.address)}</td><td class="directory-actions"><button class="btn directory-edit" data-person="${esc(name)}" type="button">${esc(t('directory.edit'))}</button>${canEdit() ? `<button class="btn directory-departments" data-person="${esc(name)}" type="button">${esc(t('directory.departments'))}</button>` : ''}<button class="btn directory-attendance" data-person="${esc(name)}" type="button">${esc(t('directory.attendance'))}</button></td></tr>`;
  }).join('');
  body.querySelectorAll('.directory-edit').forEach(button => button.addEventListener('click', event => {
    event.stopPropagation();
    const search = overlay.querySelector('#directorySearch')?.value || '';
    showPersonnelDetail(button.dataset.person, () => { renderRows(search); directoryUpdate(); });
  }));
  body.querySelectorAll('.directory-departments').forEach(button => button.addEventListener('click', event => {
    event.stopPropagation();
    showPersonnelDepartments(button.dataset.person, () => { renderRows(overlay.querySelector('#directorySearch')?.value || ''); directoryUpdate(); });
  }));
  body.querySelectorAll('.directory-attendance').forEach(button => button.addEventListener('click', event => {
    event.stopPropagation();
    showPersonnelAttendance(button.dataset.person);
  }));
  return filtered;
}

export function showPersonnelDirectory(onUpdate = () => {}, onClose = () => {}) {
