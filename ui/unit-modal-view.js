  ...
  list.innerHTML = units.map(u => {
    ...
    return `
      <div class="unit-row" style="display:flex;align-items:center;gap:8px;padding:8px;border-radius:8px;background:rgba(255,255,255,0.05);margin-bottom:6px">
        <span style="flex:1;font-size:13px;color:#fff;font-weight:${isCurrent ? 600 : 400}">${esc(u.name)}${isCurrent ? ' ✓' : ''}</span>
        <button class="action-btn" data-edit="${esc(u.id)}" aria-label="${esc(t('directory.edit'))}">✎</button>
        <button class="action-btn action-btn-danger" data-delete="${esc(u.id)}" aria-label="${esc(t('modal.delete'))}">×</button>
      </div>
    `;
  }).join('');
