import { getPersonnelList, getWeeklyTotal, getMonthlyTotal, getScheduleData, getPersonnelType, getDutyRecords } from '../state.js';
import { getWeeks, SHIFT_HOURS, normalizeShiftCode, t } from '../utils.js';
import { getDutyWarningSummary, warningText } from './duty-conflict-utils.js';
...
function checkDutyWarnings() {
  return getDutyWarningSummary(getDutyRecords()).map(entry => ({
    type: entry.warning.type === 'overlap' ? 'duty-overlap' : 'duty-rest',
    detail: warningText(entry)
  }));
}

function checkAvailabilityWarnings() {
  const warnings = [];
  const schedule = getScheduleData();
  getPersonnelList().forEach(name => {
    for (let day = 1; day <= 31; day += 1) {
      const status = getAvailabilityStatus(name, day);
      const code = String(schedule[name]?.[String(day)] || '').trim();
      if (['available', 'preferred'].includes(status) || !code || code === 'İ' || code === 'R' || code === 'ÜY') continue;
      warnings.push({ type: 'availability', detail: t('availability.warningDetail', { name, day, status: t(`availability.status.${status}`) }) });
    }
  });
  getDutyRecords().forEach(record => {
    const status = getAvailabilityStatus(record.person, record.day);
    if (record.person && !['available', 'preferred'].includes(status)) {
      warnings.push({ type: 'availability', detail: t('availability.dutyWarningDetail', { name: record.person, day: record.day, status: t(`availability.status.${status}`) }) });
    }
  });
  return warnings;
}
...
export function getWarningSummary() {
  const groups = [checkOvertimeWarnings(), checkRestWarnings(), checkLeaveWarnings(), checkCodeWarnings(), checkDutyWarnings(), checkAvailabilityWarnings()];
  return { total: groups.reduce((sum, items) => sum + items.length, 0), groups: groups.filter(items => items.length).length };
}
...
export function showWarningsPanel() {
  ...
  const duty = checkDutyWarnings();
  const availability = checkAvailabilityWarnings();
  ...
  overlay.innerHTML = `
    <div class="modal glass" style="max-width:520px;max-height:80vh;display:flex;flex-direction:column">
      ...
      <div style="overflow-y:auto;flex:1;padding-right:4px">
        ...
        <div style="margin-top:14px">
          <p class="modal-label" style="margin-bottom:8px">◈ ${t('warnings.duty')}</p>
          ${renderWarnings(duty, t('warnings.noDuty'))}
        </div>
        <div style="margin-top:14px">
          <p class="modal-label" style="margin-bottom:8px">◷ ${t('warnings.availability')}</p>
          ${renderWarnings(availability, t('warnings.noAvailability'))}
        </div>
      </div>
