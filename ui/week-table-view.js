...
function punchValueFor(records, schedule, name, day) {
  ...
  if (duty) return punchCodeForDuty(duty);
  const saved = (schedule[name] && schedule[name][day]) || '';
  if (saved) return saved;
  return { annual: 'İ', sick: 'R', unpaid: 'ÜY' }[getAvailabilityStatus(name, day)] || '';
}
...
function refreshRow(row, days) {
  ...
  const required = getRequired(days, pType, name);
...
function refreshMonthlyRow(name) {
  try {
    ...
    const eligibleDays = allDays.filter(day => !['annual', 'sick', 'unpaid'].includes(getAvailabilityStatus(name, day)));
    const required = Math.round(eligibleDays.length * getDailyRequired(pType));
...
export function renderWeekTable(tableContainer, weekLabel, onUpdate) {
  ...
  tableContainer.querySelectorAll('.day-input[data-day]').forEach(input => {
    input.addEventListener('input', () => {
      ...
      input.value = value;
      setAvailabilityStatus(name, day, availabilityStatusForCode(value) || 'available');
