...
// Dönem açılışta cihazın yerel tarihine göre otomatik başlar.
// Böylece yeni ay başladığında uygulama eski ayı yanlışlıkla açmaz.
...
export function setPeriod(year, month) {
  ...
  if (!Number.isInteger(nextYear) || !Number.isInteger(nextMonth) || nextMonth < 0 || nextMonth > 11) return;
  _year = nextYear;
  _month = nextMonth;
}
...
export function isCurrentPeriod(year = _year, month = _month) {
...
export function getFullDayName(d) {
  return DAYS_FULL_TR[new Date(_year, _month, d).getDay()];
}
...
export function t(key, values) {
  try {
    const platformValue = window.miniappI18n?.t?.(key, values);
    if (typeof platformValue === 'string'
      && platformValue.trim().length > 0
      && platformValue !== key
      && platformValue !== String(key).split('.').pop()) {
      return platformValue;
    }
  } catch {
    // Fall through to the bundled desktop/browser catalog.
  }
  return desktopTranslate(key, values);
}
